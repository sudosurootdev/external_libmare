...
atomicops<N,T>::load_unchecked(T const* src, T* dest, std::memory_order m)
{
  ...
  case 4:
    {
      ...
      // I want to use:
      //   *des = __sync_fetch_and_or(ptr, 0);
      // but it always writes, I do wonder which is better.
  ...
  case 16:
    ...
    break;
#endif  // MARE_HAS_ATOMIC_DMWORD && MARE_SIZEOF_MWORD == 8
  default:
    MARE_UNREACHABLE("unsupported size for atomicops");
    break;
  }
#endif // ! MARE_USE_GCC_SYNC_ATOMICS
  // GCC claims these are set but unused
  MARE_UNUSED(src);
}
